...
int demultiplex_packet(char * buffer,struct sockaddr_in * udp_remote)
{
    ...
    /* set the sockaddr of the remote simptcp socket from which the packet originates*/
    ...
    for (fd=0; fd<simptcp_entity.open_simptcp_sockets; fd++)
    {
        if((sock=simptcp_entity.simptcp_socket_descriptors[fd]) != NULL)
        {
            ...
            { /* this is the fetched listening socket */
                 ...
                 * save the remote udp/simpTCP addresses; they will be used when processing the received pdu
	            */
...
void * simptcp_entity_handler()
{
    ...
    while (1)
    {
		...
		if (simptcp_entity.in_len != -1) {
            DPRINTF("************************************************************\n");
			DPRINTF("Received packet of size %d on %s:%hu\n", simptcp_entity.in_len,
                inet_ntoa(udp_remote.sin_addr),
...
int start_simptcp(int local_udp)
{
	 ...
	 */
    res = pthread_create(&(simptcp_entity.simptcp_handler), NULL, simptcp_entity_handler, NULL);
    if (res < 0) {
